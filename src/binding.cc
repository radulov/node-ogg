...
NAN_METHOD(node_ogg_sync_write) {
  ...
  v8::Maybe<double> maybeSize = info[2]->NumberValue(Nan::GetCurrentContext());
  double size = maybeSize.FromMaybe(0);
...
NAN_METHOD(node_ogg_stream_init) {
  ...
  v8::Maybe<int64_t> maybeSerialno = info[1]->IntegerValue(Nan::GetCurrentContext());
  int64_t serialno = maybeSerialno.FromMaybe(0);
...
#define SIZEOF(value) \
  ...
  Nan::Set(target, Nan::New<String>("ogg_packet_replace_buffer").ToLocalChecked(),
    Nan::New<FunctionTemplate>(node_ogg_packet_replace_buffer)->GetFunction(Nan::GetCurrentContext()).ToLocalChecked());
